...
::::{dropdown} ERROR MESSAGE: `Usage: dwi-preproc|dwi-tracto <specfile>` and immediate exit
...
::::{dropdown} ERROR MESSAGE: `Error: Variable '<x>' is not set or is empty.`
...
::::{dropdown} ERROR MESSAGE: `no dwi scan/bvec found for <subj> - <session>`
...
::::{dropdown} ERROR MESSAGE: `no TotalReadOutTime or PhaseEncodingDirection found in dwi json file`
...
::::{dropdown} ERROR MESSAGE: `PE directions of dwi and fmap are NOT opposites`
The field map(s) in `fmap/` don't have a phase-encoding direction
opposite to the DWI acquisition, or the `IntendedFor` linking is wrong.
