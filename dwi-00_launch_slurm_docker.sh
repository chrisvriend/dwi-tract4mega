...
# ml docker   # uncomment/adjust if docker is provided as an environment module
...
# basedir is where we're writing spec_${subj}.json. Docker does NOT
...
# bind mounts (docker -v flags): HOST path : CONTAINER path
...
# Run as the submitting user so output files aren't root-owned.
...
# ============================================================
cmd_qc="docker run --rm ${useropt} ${mountargs} ${containerimage} dwi-qc ${subjspecjson}"
