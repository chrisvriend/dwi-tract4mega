...
Usage() {
    echo "Usage: $0 -i <bidsdir> -o <outputdir> -w <workdir> -s <subj> -c <scriptdir> [-z <session>]"
    exit 1
}
...
if [[ ! -f ${bidsdir}/${subj}${sessionpath}dwi/${subj}${sessionfile}dwi.nii.gz || ! -f ${bidsdir}/${subj}${sessionpath}dwi/${subj}${sessionfile}dwi.bvec ]]; then
    log "${RED}" "no dwi scan/bvec found for ${subj} - ${session}"
    exit 1
fi

log "${YELLOW}" "----------------------"
log "${YELLOW}" "Preprocessing dwi data"
log "${YELLOW}" "${subj}"
log "${YELLOW}" "${session}"
log "${YELLOW}" "----------------------"
...
if [ -z ${dwi_trt} ] || [ -z ${dwi_PE} ]; then
    log "${RED}" "no TotalReadOutTime or PhaseEncodingDirection found in dwi json file"
    echo 
    continue
fi
...
if [ -d ${fieldmap_folder} ]; then
    ...
    log "${YELLOW}" " fieldmap folder found"

    mkdir -p ${workdir}/${subj}${sessionpath}fmap

    # if [[ $(ls ${fieldmap_folder}/*acq-dwi*dir*epi.json | wc -l) -gt 2 ]]; then
    #     log "${RED}" "ERROR! more than 2 fieldmaps for acq-dwi found in ${fieldmap_folder}"
    #     exit 0
    # fi
    ...
    for fmap_json in ${fieldmap_folder}/*acq-dwi*dir*epi.json; do
        if [[ $(jq -r '.IntendedFor' "${fmap_json}") == *dwi* ]]; then
            ...
            echo "${fmap_json}"
            log "${BLUE}" "PhaseEncodingDirection: $fmap_PE"
            log "${BLUE}" "TotalReadoutTime: $fmap_trt"
            echo
            if [[ -f ${fmap_nii} ]];then
...
if [ ${#fmap_samePE[@]} -eq 0 ] && [ ${#fmap_otherPE[@]} -ne 0 ]; then
    ...
    log "${BLUE}" " one fieldmap available in fmap folder"
    echo
    ...
    if [[ "$dwi_PE" == "$opposite_pe1" ]]; then
        log "${GREEN}" " PE directions of dwi and fmap are opposites."
    else
        log "${RED}" "PE directions of dwi and fmap are NOT opposites."
        continue
    fi
    ...
    if [[ "$PE_same" == "$opposite_pe1" ]]; then
        log "${GREEN}" "The fmap PE directions are opposites."
    else
        log "${RED}" "The fmap PE directions are NOT opposites."
        continue
    fi
    ...
    if [[ "$dwi_PE" == "$PE_same" ]]; then
        log "${GREEN}" " PE directions of dwi and 'same' fmap are consistent."
    else
        log "${RED}" "PE directions of dwi and fmap are NOT opposites."
        continue
    fi
    ...
    # Check if the second direction matches the opposite of the first
    if [[ "$dwi_PE" == "$opposite_pe1" ]]; then
        log "${GREEN}" " PE directions of dwi and 'opposite' fmap are consistent."
    else
        log "${RED}" "PE directions of dwi and fmap are NOT opposites."
        continue
    fi
    ...
    log "${BLUE}" "merge blip up/down scans for topup"
...
# elif [ -z ${fmap_samePE} ] && [ -z ${fmap_otherPE} ]; then
    ...
    log "${BLUE}" "no fmaps found - creating syn b0 for topup"
	 ...
	 # skull-strip T1w for synb0 input
  	...
  	if [[ ! -f ${workdir}/${subj}${sessionpath}fmap/synb0/output/b0_d_smooth.nii.gz ]] || \
        ...
        if [[ -f ${workdir}/${subj}${sessionpath}fmap/${subj}${sessionfile}dir-${dwidir}${otherdir}_space-dwi_desc-4topup_epi.nii.gz ]]; then
            ...
            log "${RED}" "something went wrong with synb0"
            exit 1
        fi
    fi 
...
log "${GREEN}" "Preprocessing complete."
exit 0
