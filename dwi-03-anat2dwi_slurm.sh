...
usage() {
    echo "Usage: $0 -i <bidsdir> -o <outputdir> -w <workdir> -f <freesurferdir> -s <subj> [-z <session>]"
    exit 1
}

while getopts ":i:o:w:f:z:s:" opt; do
    case $opt in
        i) bidsdir="$OPTARG" ;;
        o) outputdir="$OPTARG" ;;
...
if [[ -d "${freesurferdir}/${subj}" && ! -f "${freesurferdir}/${subj}/scripts/T1w-2-dwi.done" ]]; then
    ...
    if [[ ! -f "${outputdir}/dwi-preproc/${subj}/anat/${subj}_res-FS_desc-5tt-hsvs_probseg.nii.gz" ]] ||
        ...
        rsync -av ${outputdir}/dwi-preproc/${subj}/anat/${subj}_res-FS_*probseg.nii.gz ${workdir}/${subj}/anat/
...
#     mri_ca_label -threads ${threads} "${SUBJECTS_DIR}/${subj}/mri/brain.mgz" \
