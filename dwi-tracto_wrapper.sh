...
if [[ -z "${session}" ]]; then
  ...
  log "$BLUE" "Step 04a (see log file in ${outputdir}/dwi-tracto/${subj}/log)"
  ...
  if [[ $status -eq 0 ]]; then
    ...
    log "$BLUE" "Steps 04b tracts2conn (see log file in ${outputdir}/dwi-tracto/${subj}/log)"
